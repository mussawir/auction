    ...
    <div class="panel-body">
        <div data-scrollbar="true" data-height="300px">
            <table id="exercise-data-table" class="table table-striped table-hover">
                ...
                <tbody>
                @foreach($pro_sug_master as $item)
                <tr>
                    <td>{{date('m/d/Y H:i:s', strtotime($item->created_at))}}</td>
                    <td>{{$item->pra_fullname}}</td>
                    <td>{{$item->message}}</td>
                    <td>
                        <button type="button" class="btn btn-sm btn-info" onclick="window.location.href='{{url('/patient/index/product-suggestion-details/'.$item->id)}}'">View</button>
                    </td>
                </tr>
                @endforeach
                </tbody>
