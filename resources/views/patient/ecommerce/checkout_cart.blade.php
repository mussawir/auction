    ...
    <div class="section-container" id="checkout-cart">
        ...
        <div class="checkout">
                    ...
                    @if(count($products))
                        <table class="table table-cart" id="cart_table">
                            ...
                            @foreach($products as $product)
                                ...
                                <tr id="calculations" class="calculations-{{ $product->cart_id }}">
									...
									<td id="cart_pra_discount<?php echo '_'.$count; ?>" class="cart-price text-center">@if($discount > 0)
									{{ $discount }}
									@else
										No Discount
									@endif</td>
                                    <td class="cart-qty text-center">
                                        <div class="cart-qty-input">
                                           <!-- <a href="#"  onclick="$('<?php echo $qtyId; ?>').val($('<?php echo $qtyId; ?>').val()!=1?parseInt($('<?php echo $qtyId; ?>').val())-1:1);calculateFinalTotal();" class="qty-control left disabled" data-click="decrease-qty" data-target="#qty"><i class="fa fa-minus"></i></a>-->
                                            ...
                                            <input type="number" onblur="calculateFinalTotal();" onchange="calculateTotal('<?php echo str_replace("#","",$qtyId);?>');calculateFinalTotal();"  value="{{ $product->qty }}" name="quantity[]" class="form-control" id="<?php echo str_replace("#","",$qtyId);?>" style="width:50px;" min="0"/>
                                    ...
                                    <td class="text-center">
                                        <a href="javascript:void(0);" onclick="doDelete('{{$product->cart_id}}', '{{ $product->products_id }}',this);" style="border: none; background-color: transparent; color: red; font-size: 15px;"><i class="fa fa-trash-o"></i> Delete</a>
                                    </td>
                                ...
                                <td class="cart-summary" colspan="6">
                                    <div class="summary-container">
                                        ...
                                        <div class="summary-row total">
                                            ...
                                            <div class="value" id="cart-total">${{ round($totalprice, 2) }}</div>
                                        </div>
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="checkout-footer">
                ...
                @if(count($products))
                    {!! Form::submit('Checkout', array('class'=>'btn btn-inverse btn-lg p-l-30 p-r-30 m-l-10','id'=>'btn_submit')) !!}
                @else
