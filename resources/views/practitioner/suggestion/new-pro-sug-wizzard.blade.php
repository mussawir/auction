                ...
                <div class="panel-body">
                    ...
                    <div class="form-group">
                        ...
                        <select style="width: 111%;" id="p-group" class="form-control" onchange="childFormSubmit(1,this.value);">
							...
							@foreach($productGroup as $productGroupPrint)
								<option value="{{$productGroupPrint->cg_id}}">{{$productGroupPrint->name}}</option>
							@endforeach
							@endif
						</select>
                        </div>
                                                        ...
                                                        <!--@foreach($products as $item)
                                                           <tr>
                                                                ...
                                                                @if(isset($item->mainImage) && (!empty($item->mainImage)))
																	<img src="{{asset($item->mainImage)}}" alt="{{$item->products_name}}" style="    width: 50px;"/>
																@else
                                                                ...
                                                                <td>
                                                                    <div class="checkbox">
                                                                        <label>
                                                                            <input type="checkbox" name="pro_id[]" data-parsley-required="true" value="{{$item->products_id}}" {{in_array($item->products_id, $pro_ids)? 'checked="checked"' : ''}}>
                                                                        </label>
                                                                    </div>
                                                                </td>
                                                            </tr>
    ...
    <div id="circleG_3" class="circleG"></div>
                                                ...
                                                <table id="selected-table-sup" class="table table-striped table-hover">
                                                    ...
                                                    @foreach($selected_pro as $item)
                                                        ...
                                                        <td style="display: none">
                                                            ...
                                                            <td>
                                                                @if(isset($item->mainImage) && (!empty($item->mainImage)))
                                                                    <img src="{{asset('$item->mainImage')}}" alt="{{$item->products_name}}" class="img-responsive" style="max-height: 64px;" />
                                                                @else
                                                                    <img src="{{asset('public/dashboard/img/no_image_64x64.jpg')}}" alt="{{$item->products_name}}" />
                                                                @endif
                                                            </td>
                                                            ...
                                                            <td>{{$item->cat_name}}</td>
                                                            <td>
                                                                {{--<input type="hidden" name="pro_id[]" value="{{$item->products_id}}" />--}}
                                                                <!--<a href="javascript:void(0);" class="text-danger" onclick="removeSupRow(this, '{{$item->products_id}}')"><i class="fa fa-times"></i> Remove</a>-->
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                    </tbody>
                    ...
                    <div class="form-group">
                        ...
                        <select style="width: 111%;" id="p-group" class="form-control" onchange="childFormSubmit(2,this.value);">
                            <option value="">Please Select</option>
							@if(isset($patientGroup))
							@foreach($patientGroup as $patientGroupPrint)
								<option value="{{$patientGroupPrint->cg_id}}">{{$patientGroupPrint->name}}</option>
							@endforeach
							@endif
						</select>
                        </div>
                                                        ...
                                                        <td style="display: none">
                                                            ...
                                                            <td>
                                                                @if(isset($item->main_image) && (!empty($item->main_image)))
                                                                    <img src="{{asset('public/dashboard/img/sup-img/'.$item->main_image)}}" alt="{{$item->products_name}}" class="img-responsive" style="max-height: 64px;" />
                                                                @else
                                                                    <img src="{{asset('public/dashboard/img/no_image_64x64.jpg')}}" alt="{{$item->products_name}}" />
                                                                @endif
                                                            </td>
...
@foreach($selected_patients as $patient)
                            ...
                            <td style="display: none">
                                ...
                                <td>
                                    ...
                                    <!--<a href="javascript:void(0);" class="text-danger" onclick="removePatientRow(this, '{{$patient->pa_id}}')"><i class="fa fa-times"></i> Remove</a>-->
                                </td>
                            </tr>
                        @endforeach
                                                        </tbody>
...
function addSuppementsList()
{
    ...
    $.ajax({
        type: "POST",
        data: form_data,
        url: '{{ URL::to('/practitioner/suggestion/add-pro-wizz') }}',
        ...
        success: function (result) {
            ...
            for(var i = 0;i<mainsplit.length;i++)
            {
                ...
                imgAppend += 'alt="{{$item->products_name}}" class="img-responsive" style="max-height: 64px;" />';
                appender+='<tr>';
...
$('#patient-btn-1').hide();
    ...
    $.ajax({
        type: "POST",
        data: form_data,
        url: '{{ URL::to('/practitioner/suggestion/add-pro-patient-wizz') }}',
        beforeSend: function (request) {
            return request.setRequestHeader('X-CSRF-Token', $("meta[name='csrf-token']").attr('content'));
        },
        success: function (result) {
...
function saveGroup()
        {
            ...
            $.ajax({
                type: "POST",
                data: form_data,
                url: '{{ URL::to('/practitioner/suggestion/save-pro-wiz') }}',
                beforeSend: function (request) {
                    return request.setRequestHeader('X-CSRF-Token', $("meta[name='csrf-token']").attr('content'));
                },
                success: function (result) {
		...
		function childFormSubmit(types,cg_ids)
		{
			...
			async:true,
			url     : '{{ URL::to('/practitioner/suggestion/groupData') }}',
			beforeSend: function (request) {
            return request.setRequestHeader('X-CSRF-Token', $("meta[name='csrf-token']").attr('content'));
        },
			success: function (result) {
